...
tensor* backward_error_loop(tensor* truths, tensor* outputs, int batch_size, double invert_batch_size, double invert_output_size, loss* loss)
{
    ...
    for(int i=0;i<batch_size;i++)
    {
        loss->loss_prime(&truths[i], &outputs[i],mean_gradients, batch_size, invert_batch_size, invert_output_size);
    }
...
tensor* loss_prime_cce(tensor* truth, tensor* output, tensor* gradient, int batch_size, double invert_batch_size, double invert_output_size)
{
    for(int i=0;i<output->size;i++)
    {
        ...
        gradient->v[i]=-(truth->v[i]/d)*invert_batch_size;
    }
    return gradient;
}
...
tensor* loss_prime_mse(tensor* truth, tensor* output, tensor* gradient, int batch_size, double invert_batch_size, double invert_output_size)
{
    ...
    {
        double d = output->v[i]==0?1:output->v[i];
        gradient->v[i]=2*(output->v[i] - truth->v[i])*invert_batch_size*invert_output_size;
    }
    return gradient;
}
